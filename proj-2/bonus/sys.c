...
/*
*  os proj required part.
*
*/

SYSCALL_DEFINE3(set_mm_limit, uid_t , uid , unsigned long , mm_max, unsigned int, time )
{
	...
	list_for_each_entry(mm,&mmlimit_struct.list,list){
		...
		printk("uid=%d, mm_max=%lu, time=%u\n",mm->uid,mm->mm_max,mm->time);
	...
	if(!is_exist)
	{
		...
		printk("uid=%d,mm_max=%lu, time=%u\n",new_tmp->uid , new_tmp->mm_max, new_tmp->time);
...
SYSCALL_DEFINE1(check_oom_per_time, unsigned int , start_time){
	...
	printk("Syscall-386 is called\n");
	oom_killer_per_time(start_time);
