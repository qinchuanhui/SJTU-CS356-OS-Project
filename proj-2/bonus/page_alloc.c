...
/* 
 *
 *  my implementation begins from now
 *
 *
 */

// choose a proper process to kill 
	...
	p = &init_task ;
		for_each_process (p){
			...
			if(curr_uid == needed_uid)
			{
				...
				{	
					//The baseline is the proportion of RAM that each task's rss, pagetable and swap space use.
					points = get_mm_rss(p->mm) + p->mm->nr_ptes;
					points += get_mm_counter(p->mm, MM_SWAPENTS);
					points *= 100;
				}
				// consider oom_socre_adj
				points += p->signal->oom_score_adj;

				// if p has lots of switches, it may be frequently executed for a long time, then we should not kill it.  
				points -= p->nvcsw + p->nivcsw;

				// utime and stime measure the age of a process, the earlier created p should be killed.
...
// a timer function  to release the user from mask
...
// this functino is called in new_ommkiller. 
void handle_each_user(struct MmlimitStruct *mm_limit){	
		...
		*/
		if(tot_size >  mm_limit->mm_max ){	
				...
				if(!mm_limit->flag)
						{	
							printk("Firstly find the oom for uid=%u, and wait some time\n",needed_uid);
							mm_limit->flag=1;
							//set the mask;
							mm_limit->mask=1;
							...
							mm_limit->user_timer.expires=jiffies+ delay_time*HZ/1000;
				...
				printk("After the allowed_time, uid=%u still exceeds\n",needed_uid);
				...
				*/
				...
				mm = victim->mm;
				printk(KERN_INFO "NEW-OOM-Killer  Choosen process info: uid=%d,"
				"uRSS=%lu, mm_max=%lu, pid=%d, pRSS=%lu", needed_uid, tot_size, mm_limit->mm_max, victim->pid, get_mm_rss(mm)<<PAGE_SHIFT);
				task_unlock(victim);

				// Kill all user processes sharing victim->mm in other thread groups, if any.
				for_each_process(max_pro)
					if(max_pro->mm==mm && !same_thread_group(max_pro, victim) && !(max_pro->flags & PF_KTHREAD)){
						...
						task_unlock(max_pro);
						//send signal to kill
						do_send_sig_info(SIGKILL, SEND_SIG_FORCED, max_pro, true);
...
// new_oomkiller for basic requirement
